...
def handle_command(
    ...
    if parsed.cmd_name == "publish":
        ...
        ok = conn.root.publish(id, parsed.args[0], parsed.args[1])
        if ok:
            print("=> Ok!", file=output)
        else:
            print("=> Not ok!", file=output)
    elif parsed.cmd_name == "subscribe":
        assert len(parsed.args) == 1
        ok = conn.root.subscribe_to(id, parsed.args[0])
        if ok:
            print("=> Ok!", file=output)
        else:
            print("=> Not ok!", file=output)
        ...
        assert len(parsed.args) == 1
        ok = conn.root.unsubscribe_to(id, parsed.args[0])
        if ok:
            print("=> Ok!", file=output)
        else:
            print("=> Not ok!", file=output)
        ...
        topics = conn.root.list_topics()
        print(f"=> topics: {topics}", file=output)
    elif parsed.cmd_name == "refresh":
        assert len(parsed.args) == 0
        topics = conn.root.list_topics()
        print(f"=> refreshed", file=output)
...
def main() -> int:
    ...
    with create_conn(HOST, PORT) as conn:
        ...
        loginok: bool = conn.root.login(id, callback)
        if loginok:
            print("=> Login ok!", file=outfile)
        else:
            print("=> Login not ok!", file=outfile)
            return 1
        ...
        while not should_stop:
            while len(mail) > 0:
                ...
                print(f"New message: {content}", file=outfile)
            parsed: Optional[ParsedCommand] = ClientCli.command(infile, outfile)
            if parsed is None:
                # Nothing to do
                pass
            else:
